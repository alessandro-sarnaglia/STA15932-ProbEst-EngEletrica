...
<ol>
  <li>
  Vimos que, se o modelo probabilístico é concebido sobre um espaço amostral $\Omega$ abstrato e $X : \Omega \to \mathbb{R}$, então a fmp pode ser obtida por
$$p_X(x) = P(X = x) = P(A_x),$$
em que $A_x = \{\omega \in \Omega : X(\omega) = x\}$. Isto é, a fmp de $X$ é **induzida** pela medida de probabilidade em $\Omega$;
  </li>
  <li class="fragment">
  Muito frequentemente, a construção de um modelo probabilístico em $\Omega$ é muito complicada, o que dificultaria a determinação da fmp induzida.
  </li>
  <li class="fragment">
  Para contornar esse problema, levando em consideração o comportamento da vad em estudo, podemos especificar diretamente uma fmp para a vad, desde que atenda as propriedades já vistas:
    <ul>
      <li>
      $p_X(x) \geq 0$;
      </li>
...
<ol>
  <li>
    A fórmula para o cálculo de $\mu$ é muito similar àquela vista em estatística descritiva, no entanto, agora a fmp $p_X(x;\theta)$ toma o lugar da frequência relativa que teria sido efetivamente observada;
  </li>
  <li class="fragment">
    De maneira similar a estatística descritiva, $\mu$ é calculada como uma média **ponderada** com pesos dados por $p_X(x;\theta)$;
  </li>
  <li class="fragment">
    Naturalmente, para famílias de fmp's, pelo fato de $p_X(x;\theta)$ ser indexada por $\theta$, a sua esperança também será função de $\theta$.
  </li>
</ol>
...
<ol>
  <li>
    Quando está claro a que va $X$ a esperança se refere, usaremos $\mu$ em vez de $\mu_X$;
  </li>
  <li class="fragment">
    No Exemplo 4, se hipotetizarmos uma população em que a vad $X$ com valores $0, 1$ e $2$ associados as frequências relativas $\alpha$, $\beta$, $1-\alpha-\beta$, respectivamente, então $\mu = 2-2\alpha - \beta$ será a média populacional. Por isso podemos nos referir a $\mu$ por média populacional ou teórica;
  </li>
  <li class="fragment">
    No Exemplo 4, tomando $\theta = (\frac{1}{2}, \frac{1}{3})$, obtemos $\mu = 2-2\frac{1}{2} - \frac{1}{3} = \frac{2}{3} \approx 0.67$, que não é um valor possível de $X$. A palavra esperado deve ser interpretada com cautela porque uma pessoa não espera ver um valor de $X = 0.67$ quando um único estoque é selecionado.
  </li>
</ol>
...
<ol>
  <li>
    Seja $X$ vad com fmp $p_X(x)$ e esperança $\mu$. Então
$$\begin{align*}
...
<ol>
  <li>
    No Exemplo 9, definimos como sucesso o aluno acertar a questão, mas esse não precisa ser o caso. A classificação do que será entendido como **sucesso** dependerá do problema em questão.
  </li>
  <li class="fragment">
    O experimento de Bernoulli pode ser utilizado para construção de vad's mais complexas.
  </li>
</ol>
...
<ol>
  <li>
    No Exemplo 12, temos que $X \sim \text{BN}(k=3, p)$. Se $p = 0.01$, por exemplo, teríamos $\mu = E(X) = \frac{3}{0.01} = 300$, $\sigma^2 = V(X) = 3\frac{0.99}{0.01^2} = 29700$ e $\sigma \approx 172.34$. Em média, são realizadas $300$ transações até a terceira falha com DP de $172.34$. Além disso, $P(X > \mu) = P(X > 300) \approx 0.4221$;
  </li>
  <li class="fragment">
    A distribuição BN também pode ser definida como a distribuição do número de fracassos (em vez de repetições) até o $k$-ésimo sucesso. Nesse caso, sua fmp é
...
e sua esperança e variância são, respectivamente, $\mu = k\frac{(1-p)}{p}$ e $\sigma^2 = k\frac{(1-p)}{p^2}$;
  ...
  <li class="fragment">
    Se $X \sim \text{BN}(k = 1, p)$, dizemos que $X$ tem distribuição geométrica, cuja notação é $X \sim \text{Geo}(p)$;
  </li>
  <li class="fragment">
    Se $X \sim \text{BN}(k, p)$, então $X = \sum_{j=1}^k X_j$, em que $X_1,\ldots,X_k \sim \text{Geo}(p)$ independentes.
  </li>
  <li class="fragment">
    Bin: repetições fixas e sucessos aleatórios; BN: sucessos fixos e repetições aleatórias.
  </li>
</ol>
...
<ol>
  <li>
    Note que passamos de um problema de calcular as probabilidades com respeito a $X \sim \text{N}(\mu, \sigma^2)$, com $\mu$ e $\sigma^2$ quaisquer, para as probabilidades de uma **normal padrão** $Z \sim \text{N}(0,1)$;
  </li>
  <li class="fragment">
    As probabilidades para $Z \sim \text{N}(0,1)$ também não tem solução analítica, mas métodos numéricos foram utilizados para computar e tabelar $\Phi(z)$ para diversos valores de $z$.
  </li>
</ol>
...
<ol>
  <li>
    Em inferência, veremos um dos teoremas mais importantes para utilização em aplicações, o **Teorema Central do Limite** (TCL). Basicamente, o TCL afirma que, em grandes amostras, a média amostral tem distribuição aproximadamente normal.
  </li>
  <li class="fragment">
    Como a média amostral é soma de va's dividida pelo tamanho da amostra, é natural, imaginar que, se uma va $X$ pode ser enxergada como soma de várias va's, então a sua distribuição também será bem aproximada por uma normal. Esse é o caso da distribuição Binomial (soma de Bernoulli's), por exemplo.
  </li>
</ol>
...
<ol>
  <li>
    Note que, no Exemplo 15, o cálculo do valor exato de $P(X \leq 10)$ envolveu o cálculo de 11 probabilidades da distribuição binomial, o que pode ser exaustivo se realizado sem auxílio computacional;
  </li>
  <li class="fragment">
    A alternativa que surge é utilizar $Y \sim \text{N}(\mu, \sigma^2)$ como aproximação para $X \sim \text{Bin}(n,p)$;
  </li>
  <li class="fragment">
    Os parâmetros $\mu$ e $\sigma^2$ da distribuição de $Y$ utilizada para a aproximação são $\mu = \mu_X = np$ e $\sigma^2 = np(1-p)$, que são a média e a variância da $X \sim \text{Bin}(n,p)$;
  </li>
  <li class="fragment">
    Tendo em vista que $X$ pode ser representada como soma de $n$ va's $\text{Ber}(p)$, o TCL garante que a aproximação do Item 2 fica melhor a medida que $n$ aumenta;
  </li>
  <li class="fragment">
    Para ilustração, os gráficos a seguir mostram a fmp de $X \sim \text{Bin}(n,p)$ com a fdp de $Y \sim \text{N}(\mu = np, \sigma^2 = np(1-p))$, para $p=0.05$ e $0.25$, e $n = 5, 10, 50$ e $100$.
  </li>
</ol>
...
<ol>
  <li>
    É possível notar que a va normal tem fdp simétrica em torno de $\mu$ com formato de sino (a fdp $f_X(x)$ decresce conforme $x$ se afasta de $\mu$).
  </li>
  <li class="fragment">
    Em muitas situações práticas, a vac é positiva e possui assimetria. Uma família de fdp com essas características é a família **gama**.
  </li>
</ol>
...
<table>
  ...
  <li class="fragment">
    Se $X \sim \text{Gama}(\alpha, \beta)$, então $\frac{X}{\beta} \sim \text{Gama}(\alpha, 1)$, isto é, $\frac{X}{\beta}$ tem distribuição gama padrão com parâmetro de forma $\alpha$.
  </li>
  <li class="fragment">
    Assim, os problemas de cálculos de probabilidades de uma $\text{Gama}(\alpha, \beta)$ podem ser transformados nos de uma $\text{Gama}(\alpha, 1)$ por meio da operação $\frac{X}{\beta}$. A probabilidades de uma gama padrão para diferentes valores de $\alpha$ podem ser obtidas por métodos numéricos e seus valores tabelados.
  </li>
</ol>
...
<ol>
  <li>
    A distribuição $\text{Exp}(\lambda)$ é um caso especial da $\text{Gama}(\alpha, \beta)$, com $\alpha = 1$ e $\beta = \frac{1}{\lambda}$;
  </li>
  <li class="fragment">
    Assim, pela esperança e variancia da distribuição gama, se $X \sim \text{Exp}(\lambda)$, temos que
<table>
  <tr style="border-bottom:none;text-align:center;">
...
<ol>
  <li>
    A distribuição exponencial é muito utilizada para modelar o tempo de vida de componentes em geral;
  </li>
  <li class="fragment">
    Uma propriedade importante da distribuição exponencial nessas aplicações é a **falta de memória**.
  </li>
</ol>
...
<ol>
  <li>
    Se $X \sim \text{Weibull}(\alpha, \beta)$ é possível mostrar que $Y = (\frac{X}{\beta})^\alpha$ satisfaz $Y \sim \text{Exp}(1)$. De modo análogo, se $Y \sim \text{Exp}(1)$, então $X = \beta Y^{1/\alpha}$ satisfaz $X \sim \text{Weibull}(\alpha, \beta)$;
  </li>
  <li class="fragment">
    Se $X \sim \text{Weibull}(\alpha, \beta)$, a fda de $X$ é
<table>
  <tr style="border-bottom:none;text-align:center;">
  ...
  <li class="fragment">
    Se $X \sim \text{Weibull}(1, \frac{1}{\lambda})$, isto é, $\alpha = 1$ e $\beta = \frac{1}{\lambda}$, então $X \sim \text{Exp}(\lambda)$.
  </li>
</ol>
...
<ol>
  <li>
    É possível mostrar que, se $X \sim \text{LogN}(\mu, \sigma^2)$, então $Y = \ln(X)$ satisfaz $Y \sim \text{N}(\mu, \sigma^2)$;
  </li>
  <li class="fragment">
...
<ol>
  <li>
    Note que $\alpha=\beta$ fornece fdp's simétricas;
  </li>
  <li class="fragment">
    Se $\alpha=\beta=1$, dizemos que $X$ tem distribuição uniforme em $(A,B)$, denotada por $X \sim \text{U}(A,B)$;
  </li>
  <li class="fragment">
    Ao trocar o valor de $\alpha$ pelo de $\beta$ e o de $\beta$ pelo de $\alpha$, a fdp é refletida em torno de $\frac{A+B}{2}$;
  </li>
  <li class="fragment">
    O cálculo de probabilidades da distribuição Beta, em geral, requer o uso de métodos numéricos. Mas, para uma Beta padrão com $\alpha, \beta \in \mathbb{N}$, é possível a sua obtenção analítica ($\Gamma(n) = (n-1)!$, $n \in \mathbb{N}$).
  </li>
</ol>
...
<ol>
  <li>
    Pela fmpc, podemos obter probabilidades conjuntas para eventos de interesse;
  </li>
  <li class="fragment">
    A fmp de apenas uma va é obtida da fmpc somando $p(x,y)$ em todos os valores da outra variável;
  </li>
  <li class="fragment">
    Nesse contexto, essa fmp univariada é chamada de fmp **marginal**.
  </li>
</ol>
...
<ol>
  <li>
    A fmpc pode ser obtida da fdac e vice-versa. A fmpc $p(x,y) = F(x,y) - F(x^-, y^-)$, que é a altura do salto de $F(\cdot,\cdot)$ em $(x,y)$ e a fdac $F(x,y)$ é obtida por $\sum_{s \leq x}\sum_{t \leq y}p(s,t)$;
  </li>
  <li class="fragment">
...
<ol>
  <li>
    A probabilidade de $(X,Y)$ pertencer a uma região de $\mathbb{R}^2$ é a integral dupla de $f(x,y)$ nessa região. Por exemplo, $P(a < X \leq b, c < Y \leq d) = \int_a^b\int_c^d f(x,y) dy dx$;
  </li>
  <li class="fragment">
    A fdpc pode ser obtida da fdac por derivação $f(x,y) = \frac{\partial^2}{\partial x \partial y} F(x,y)$;
  </li>
  <li class="fragment">
    As fdp's marginais são obtidas por integração da coordenada correspondente a outra variável:
<table style="margin-left: auto; margin-right: auto;">
  <tr style="border-bottom:none">
...
<ol>
  <li>
    Seja $(X,Y)$ vetor aleatório discreto com fmpc $p(x,y)$, então a fmp condicional de $Y$ dado que $X = x$ é dada  pela fmpc de $(X,Y)$ dividida pela fmp marginal de $X$:
<table style="margin-left: auto; margin-right: auto;">
  <tr style="border-bottom:none">
  ...
  <li class="fragment">
    Seja $(X,Y)$ vetor aleatório contínuo com fdpc $f(x,y)$, então a fdp condicional de $Y$ dado que $X = x$ é dada pela fdpc de $(X,Y)$ dividida pela fdp marginal de $X$:
<table style="margin-left: auto; margin-right: auto;">
  <tr style="border-bottom:none">
...
<ol>
  <li>
    Se $(X, Y)$ é vetor aleatório discreto. Dizemos que as vad's $X$ e $Y$ são independentes se
$$p_{Y|X}(y|x) = p_Y(y),\ \forall\ y \ \ \ \text{ou} \ \ \ p_{X|Y}(x|y) = p_X(x),\ \forall\ x.$$
  </li>
  <li class="fragment">
    Se $(X, Y)$ é vetor aleatório contínuo. Dizemos que as vac's $X$ e $Y$ são independentes se
...
<ol>
  <li>
    Duas vad's $X$ e $Y$ são independentes se, e somente, se $p(x,y) = p_X(x)\cdot p_Y(y)$, para todo $x,y$;
  </li>
  <li class="fragment">
    Duas vac's $X$ e $Y$ são independentes se, e somente, se $f(x,y) = f_X(x)\cdot f_Y(y)$, para todo $x,y$.
  </li>
</ol>
...
<ol>
  <li>
    Um experimento **Multinomial** consiste de $n$ repetições independentes de um experimento com $r$ resultados possíveis, que denominamos de categorias.
  </li>
  <li class="fragment">
    Em um experimento Multinomial, seja $X_j$ a quantidade das $n$ repetições que resultaram na categoria $j$, $j=1, \ldots, r$. Dizemos que o vetor aleatório $(X_1, \ldots, X_r)$ tem distribuição **Multinomial** e sua fmpc é
<table style="margin-left: auto; margin-right: auto;">
  <tr style="border-bottom:none">
...
<ol>
  <li>
    A covariância mede a direção esperada da relação linear entre as variáveis $X$ e $Y$;
  </li>
  <li class="fragment">
    $\text{Cov}(X,Y) \gg 0$ indica relação **linear** positiva forte;
  </li>
  <li class="fragment">
    $\text{Cov}(X,Y) \ll 0$ indica relação **linear** negativa forte;
  </li>
  <li class="fragment">
    $\text{Cov}(X,Y) \approx 0$ indica ausência de relação **linear**.
  </li>
</ol>
...
[É possível verificar que $\mu_X = 5$ e $\mu_Y = \frac{20}{16}$.]{.fragment} [Logo, $\text{Cov}(X,Y) = E(XY) - \mu_X\mu_Y = \frac{100}{16} - 5\cdot\frac{20}{16} = 0.$]{.fragment}
...
O Exemplo 23 mostra que, mesmo $X$ e $Y$ sendo [**dependentes**]{style="color:red"}, podemos obter $\text{Cov}(X,Y) = 0$. Por isso, $\text{Cov}(X,Y) = 0$ não significa independência, mas apenas indica a ausência de relação [**linear**]{style="color:red"};
...
A covariância satisfaz $-\sigma_X\sigma_Y \leq \text{Cov}(X,Y) \leq \sigma_X\sigma_Y$.
...
<ol>
  <li>
    Embora a covariância seja limitada, esses limites dependem dos desvios-padrão associados, mudando caso a caso. Assim, é difícil classificarmos o que seria uma covariância (positiva ou negativa) forte;
  </li>
  <li class="fragment">
    Uma medida mais apropriada seria o **coeficiente de correlação**.
  </li>
</ol>
...
O coeficiente de correlação coincide com cosseno do ângulo formado entre as variáveis centralizas pelas respectivas médias.
:::
...
![](figs/Ilustracao_correlacao_R3_F2_S0.drawio.svg){style="display: block; width: 100%; height: 100%; position: absolute; top: 0; left: 0;"}
:::
...
<ol>
  <li>
    Se $X$ e $Y$ são independentes, então $\text{Cov}(X, Y) = 0$ (e também $\rho = 0$). Entretanto, como visto anteriormente, $\text{Cov}(X, Y) = 0$ não implica independência entre $X$ e $Y$;
  </li>
  <li class="fragment">
    $\text{Cov}(X, X) = E[(X-\mu_X)(X-\mu_X)] = E[(X-\mu_X)^2] = V(X) = \sigma^2_X$;
  </li>
  <li class="fragment">
  ...
  <li class="fragment">
    $V(\sum_{i=1}^n X_i) = \sum_{i=1}^n \sum_{j=1}^n \text{Cov}(X_i, X_j)$.
  </li>
</ol>
