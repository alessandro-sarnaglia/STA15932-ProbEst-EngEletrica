...
**Exemplo 10 (cont.):** Vimos que $\frac{n\bar{X}}{\mu} \sim \text{Gama}(n, 1)$ para população $\text{Exp}(\lambda)$. [Suponha que desejamos testar
$$H_0 : \mu \leq \mu_0 \quad \times \quad H_1 : \mu > \mu_0.$$]{.fragment}

[Teremos evidências para rejeitar $H_0$ se $\bar{X}$ fornecer um valor significativamente maior do que $\mu_0$, o que induz à $\text{RC} = \{\bar{X} > x_c\}$.]{.fragment} [Agora, sendo $g_{p,n}$ tal que $P(\text{Gama}(n,1) \leq g_{p,n}) = p$, temos que
$$
