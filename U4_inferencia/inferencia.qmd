...
[Definindo $g_{p,\alpha}$ tal que $P(\text{Gama}(\alpha,1) \leq g_{p,\alpha}) = p$]{.fragment fragment-index=2}[, temos que]{.fragment fragment-index=3}
...
1. Uma das formas consiste de escolher um estimador $\hat{\theta}$ do parâmetro $\theta$;
...
No contexto acima, a RC de um teste de hipóteses é um conjunto de valores que, se fornecidos pelo estimador, levam à rejeição de $H_0$. Formalmente,
$$
\text{RC} = \{x : \text{se } \hat{\theta} = x, H_0 \text{ é rejeitada}\}.
$$
...
**Exemplo 11:**  Suponha que desejamos testar $H_0 : \mu \geq \mu_0$. [Uma RC natural para rejeitar $H_0$ seria $RC = \{\bar{X} < x_c\}$.]{.fragment} [Isto é, valores muito pequenos de $\bar{X}$ suportam a rejeição de $H_0$.]{.fragment} [Determinada a forma da RC, resta determinar o **valor crítico** $x_c$, o limiar que delimita a RC.]{.fragment}
...
1. Decidir entre rejeitar, ou não, $H_0$ pode acarretar em dois tipos de erros: o Erro Tipo 1 (rejeitar $H_0$, quando $H_0$ é verdadeira); e o Erro Tipo 2 (não rejeitar $H_0$, quando $H_0$ é falsa);
...
**Contextualização:** Uma indústria utiliza um parafuso que deve satisfazer determinadas especificações. Uma dessas é a resistência à tração $X$ (em $\text{kg}/\text{m}^2$). Dependendo da localidade em que os parafusos são fabricados, $X$ pode ter diferentes distribuições. Suponha que apenas as localidades $A$ e $B$ produzam os parafusos. Se o parafuso vem:

- de $A$, então $E(X) = 145$ e $\sigma^2 = 12^2$;
- de $B$, então $E(X) = 155$ e $\sigma^2 = 18^2$.

**Formulação do Teste:** Um lote grande de parafusos será leiloado e a empresa deseja comprar esse lote somente se o mesmo vier da localidade $B$. No dia do leilão, o leiloeiro divulgará a resistência à tração média de $36$ parafusos escolhidos desse lote. Podemos estabelecer as hipóteses nula e alternativa como sendo:
$$
H_0 : \mu = 155, \sigma = 18 \quad \times \quad H_1 : \mu = 145, \sigma = 12.
$$

Se decidirmos rejeitar $H_0$ quando $\bar{X} < 150$, quais as probabilidades de cometermos os Erros Tipos 1 e 2?

## {.unnumbered .unlisted .smaller} 

###  Exemplo 12 (cont.) {.unnumbered .unlisted}

Como neste caso $H_0$ e $H_1$ são ambas hipóteses simples, as probabilidades de erro $\alpha = \alpha(\mu = 155, \sigma = 18)$ e $\beta = \beta(\mu = 145, \sigma = 12)$ são de fácil obtenção.

::: columns

::: {.column #vcenter width=47% .fragment}

De um lado, a probabilidade do Erro Tipo 1 (tamanho do teste) é
$$
\begin{aligned}
\alpha &= P(\text{Erro Tipo 1}) \\
&= P(\text{Rejeitar } H_0 \mid H_0 \text{ é verdadeira}) \\
&= P(\bar{X} < 150 \mid \mu = 155, \sigma = 18) \\
&= P\left(\frac{\bar{X} - \mu}{\sigma/\sqrt{n}} < \frac{150 - 155}{18/\sqrt{36}}\right) \\
&\approx P(Z < -1\text{,}67) = 0\text{,}0475
\end{aligned}
$$

:::

::: {.column #vcenter width=6%}

:::

::: {.column #vcenter width=47% .fragment}

Por outro lado, a probabilidade do Erro Tipo 2 ($1 -$ poder do teste) é
$$
\begin{aligned}
\beta &= P(\text{Erro Tipo 2}) \\
&= P(\text{Não Rejeitar } H_0 \mid H_0 \text{ é falsa}) \\
&= P(\bar{X} \geq 150 \mid \mu = 145, \sigma = 12) \\
&= P\left(\frac{\bar{X} - \mu}{\sigma/\sqrt{n}} \geq \frac{150 - 145}{12/\sqrt{36}}\right) \\
&\approx P(Z \geq 2\text{,}5) = 0\text{,}0062
\end{aligned}
$$
...
Embora tenhamos fixado a RC e depois verificado $\alpha$ e $\beta$, o usual é especificar um $\alpha$ tolerável e encontrar a RC cuja probabilidade de Erro Tipo 1 satisfaz esse valor.
...
Primeiramente, determinamos o formato da RC. [Teremos evidências para rejeitar $H_0 : \mu = 155$ em favor de $H_1 : \mu = 145$ se $\bar{X}$ for "suficientemente" menor do que $155$, isto é, se $\bar{X} < x_c$, para algum $x_c < 155$.]{.fragment fragment-index=2}

::: columns

::: {.column #vcenter width=65% .fragment fragment-index=3}

Especificada $\text{RC} = \{\bar{X} < x_c\}$, encontramos o "valor crítico" $x_c$ cuja probabilidade de Erro Tipo 1 não supere o nível tolerado $\alpha$. Suponha $\alpha = 0\text{,}1$. Daí,
$$
\begin{aligned}
\alpha &= 0\text{,}1 = P(\text{Erro Tipo 1}) = P(\text{Rejeitar } H_0 \mid H_0 \text{ é verdadeira}) \\
&= P(\bar{X} < x_c \mid \mu = 155, \sigma = 18) = \textstyle P\left(\frac{\bar{X} - \mu}{\sigma/\sqrt{n}} < \frac{x_c - 155}{18/\sqrt{36}}\right) \\
&\approx \textstyle P\left(Z < \frac{x_c - 155}{3}\right) = P(Z < z),
\end{aligned}
$$
onde $z = \frac{x_c - 155}{3}$. [Logo, $\frac{x_c - 155}{3} = z = -1\text{,}28 \implies x_c = 151\text{,}16$.]{.fragment fragment-index=4}

:::

::: {.column #vcenter width=35% .fragment fragment-index=4}
...
# 1. Configurando os parâmetros da distribuição
alpha <- 0.1
z_critico <- qnorm(alpha) # Calcula o ponto crítico (-1.28)

# Gerando o domínio para a curva normal padrão
x <- seq(-3.5, 3.5, length = 500)
y <- dnorm(x)

# 2. Desenhando o gráfico base
plot(
  x, y, 
  type = "l", 
  lwd = 2.5, 
  ...
  xlab = "z", 
  ylab = "", 
  main = expression(paste("Região de Rejeição (Área ", alpha, " = 0.10 à Esquerda)"))
)

# Customização do eixo x incluindo o ponto z_critico
axis(1, at = c(-3, -2, z_critico, 0, 1, 2, 3), 
     labels = c("-3", "-2", "-1.28", "0", "1", "2", "3"), cex.axis = 1.1)

# 3. Colorindo a região de rejeição (cauda esquerda)
x_shading <- seq(-3.5, z_critico, length = 100)
y_shading <- dnorm(x_shading)
polygon(c(-3.5, x_shading, z_critico), c(0, y_shading, 0), col = rgb(0.23, 0.35, 0.60, 0.4), border = NA)

# Linha vertical indicativa do z crítico
abline(v = z_critico, lty = 2, col = "#3b5998", lwd = 1.5)

# 4. Adicionando rótulo para a área alpha
text(x = -2.1, y = 0.05, labels = expression(alpha == 0.10), col = "#3b5998", font = 2, cex = 1.2)
...
::: {.callout-tip .fragment fragment-index=5 style="font-size: 1.428571em;"}
## Exercício

Encontre $\beta$ para a RC obtida acima.
:::
...
<ol>
  <li>
    No exemplo anterior, como $H_0$ e $H_1$ eram simples, as probabilidades de Erros Tipos 1 e 2 possuiam apenas um valor admissível e puderam ser calculadas de maneira exata;
  </li>
  <li class="fragment">
    No entanto, se uma das hipóteses é composta ($H_0 : \theta \leq \theta_0$, por exemplo), a probabilidade do erro associado é uma função do verdadeiro valor do parâmetro;
  </li>
  <li class="fragment">
    Neste caso, optamos por ser conservadores, limitando o máximo das probabilidades de Erro Tipo 1.
  </li>
</ol>
:::
...
O **nível de significância** de um teste de hipóteses é definido como o valor máximo tolerado para a probabilidade do Erro Tipo 1. Mais precisamente, é o valor tolerado para $\sup_{\theta} \alpha(\theta)$.
:::

::: {.callout-warning .fragment}
## Propriedade

Se o tamanho do teste $\alpha(\theta)$ for função monótona de $\theta$, então $\sup_{\theta} \alpha(\theta) = \alpha(\theta_0)$ para qualquer dos formatos de hipótese nula a seguir: $H_0 : \theta = \theta_0$; $H_0 : \theta \leq \theta_0$; e $H_0 : \theta \geq \theta_0$.
:::
...
<ul>
  <li>
    Ao lado é ilustrado o tamanho $\alpha(\theta)$ de um teste com comportamento monótono;
  </li>
  <li class="fragment" data-fragment-index=3>
    Note que $\alpha(\theta)$ (área à direita de $x_c$) cresce conforme o verdadeiro $\theta$ se aproxima do limite da hipótese nula (em $\theta_0$);
  </li>
  <li class="fragment" data-fragment-index=4>
    O supremo é alcançado exatamente em $\theta = \theta_0$.
  </li>
</ul>
:::
...
#| echo: false
par(mar = c(2, 1, 1, 1))
plot(NULL, xlim = c(0, 10), ylim = c(-0.05, 0.45), axes = FALSE, xlab = "", ylab = "")
segments(x0 = 3, y0 = -0.05, x1 = 3, y1 = 0.38, lwd = 1.2, col = "gray30")
segments(x0 = 5.5, y0 = -0.05, x1 = 5.5, y1 = 0.38, lwd = 1.2, col = "gray30")
segments(x0 = 7, y0 = -0.05, x1 = 7, y1 = 0.38, lty = 2, lwd = 1.2, col = "gray30")
segments(x0 = 5.5, y0 = 0.405, x1 = 5.5, y1 = 0.425, lwd = 1.5)
arrows(x0 = 5.5, y0 = 0.415, x1 = 0, y1 = 0.415, length = 0.1, code = 3, lwd = 1)
arrows(x0 = 5.5, y0 = 0.415, x1 = 9.5, y1 = 0.415, length = 0.1, code = 3, lty = 2, lwd = 1)
text(x = 3.0, y = 0.44, labels = expression(H[0]), cex = 1.4)
text(x = 8.0, y = 0.44, labels = expression(H[1]), cex = 1.4)
text(x = 3, y = -0.03, labels = expression(theta), cex = 1.3)
text(x = 5.5, y = -0.03, labels = expression(theta[0]), cex = 1.3)
text(x = 7, y = -0.03, labels = expression(x[c]), cex = 1.3)

# Curva 1 (Valor sob H0 distante do limite)
x_grid <- seq(0, 9.5, length.out = 1000)
y1 <- dnorm(x_grid, mean = 3.0, sd = 1.5)
x_sh1 <- seq(7, 9.5, length.out = 200)
y_sh1 <- dnorm(x_sh1, mean = 3.0, sd = 1.5)
polygon(c(7, x_sh1, 9.5), c(0, y_sh1, 0), col = rgb(0.2, 0.9, 0.2, 0.8), border = NA)
lines(x_grid, y1, lwd = 2.5)

arrows(x0 = 0, y0 = 0, x1 = 10, y1 = 0, length = 0.1, lwd = 1.5, code = 2)
```
...
par(mar = c(2, 1, 1, 1))
plot(NULL, xlim = c(0, 10), ylim = c(-0.05, 0.45), axes = FALSE, xlab = "", ylab = "")
segments(x0 = 4.25, y0 = -0.05, x1 = 4.25, y1 = 0.38, lwd = 1.2, col = "gray30")
segments(x0 = 5.5, y0 = -0.05, x1 = 5.5, y1 = 0.38, lwd = 1.2, col = "gray30")
segments(x0 = 7, y0 = -0.05, x1 = 7, y1 = 0.38, lty = 2, lwd = 1.2, col = "gray30")
segments(x0 = 5.5, y0 = 0.405, x1 = 5.5, y1 = 0.425, lwd = 1.5)
arrows(x0 = 5.5, y0 = 0.415, x1 = 0, y1 = 0.415, length = 0.1, code = 3, lwd = 1)
...
text(x = 8.0, y = 0.44, labels = expression(H[1]), cex = 1.4)
text(x = 5.5, y = -0.03, labels = expression(theta[0]), cex = 1.3)
text(x = 4.25, y = -0.03, labels = expression(theta), cex = 1.3)
text(x = 7, y = -0.03, labels = expression(x[c]), cex = 1.3)

# Curvas 1 e 2 (Se aproximando do limite)
x_grid <- seq(0, 9.5, length.out = 1000)
y1 <- dnorm(x_grid, mean = 3.0, sd = 1.5)
y2 <- dnorm(x_grid, mean = 4.25, sd = 1.5)

x_sh1 <- seq(7, 9.5, length.out = 200)
y_sh1 <- dnorm(x_sh1, mean = 3.0, sd = 1.5)
polygon(c(7, x_sh1, 9.5), c(0, y_sh1, 0), col = rgb(0.2, 0.9, 0.2, 0.8), border = NA)

y_sh2 <- dnorm(x_sh1, mean = 4.25, sd = 1.5)
polygon(c(7, x_sh1, 9.5), c(0, y_sh2, 0), col = rgb(1, 0.85, 0.1, 0.6), border = NA)

lines(x_grid, y1, lwd = 2.5)
lines(x_grid, y2, lwd = 2.5)
...
text(x = 3.0, y = 0.44, labels = expression(H[0]), cex = 1.4)
text(x = 8.0, y = 0.44, labels = expression(H[1]), cex = 1.4)
text(x = 5.5, y = -0.03, labels = expression(theta==theta[0]), cex = 1.3)
text(x = 7, y = -0.03, labels = expression(x[c]), cex = 1.3)

# Curvas 1, 2 e 3 (No limite theta_0)
x_grid <- seq(0, 9.5, length.out = 1000)
y1 <- dnorm(x_grid, mean = 3.0, sd = 1.5)
y2 <- dnorm(x_grid, mean = 4.25, sd = 1.5)
y3 <- dnorm(x_grid, mean = 5.5, sd = 1.5)
...
y_sh3 <- dnorm(x_sh1, mean = 5.5, sd = 1.5)
polygon(c(7, x_sh1, 9.5), c(0, y_sh3, 0), col = rgb(0.9, 0.4, 0.4, 0.5), border = NA)

lines(x_grid, y1, lwd = 2.5)
lines(x_grid, y2, lwd = 2.5)
lines(x_grid, y3, lwd = 2.5)
...
<ol>
  <li>
    O teste de hipóteses é realizado pela construção de uma RC (critério para rejeitar $H_0$) que satisfaça o nível de significância ($\sup_\theta\alpha(\theta)$);
  </li>
  <li class="fragment" data-fragment-index=2>
    Como normalmente $\sup_\theta\alpha(\theta) = \alpha(\theta_0)$, $H_0$ sempre deve conter o ponto limítrofe ($\theta_0$);
  </li>
  <li class="fragment" data-fragment-index=3>
    O nível de significância pode ser interpretado como um trade-off entre $\alpha$ e $\beta$, pois:
    <ul>
      <li>
        $\uparrow \alpha(\theta) = P(\text{Erro Tipo 1}) \implies \downarrow \beta(\theta) = P(\text{Erro Tipo 2})$
      </li>
      <li>
        $\downarrow \alpha(\theta) = P(\text{Erro Tipo 1}) \implies \uparrow \beta(\theta) = P(\text{Erro Tipo 2})$
      </li>
    </ul>
  </li>
  <li class="fragment" data-fragment-index=4>
    Se o custo do Erro Tipo 1 for mais alto que o do Erro Tipo 2, adotamos um nível de significância baixo;
  </li>
  <li class="fragment" data-fragment-index=5>
    Se o custo do Erro Tipo 1 for mais baixo que o do Erro Tipo 2, devemos adotar um nível de significância alto (para indiretamente diminuir $\beta$);
  </li>
  <li class="fragment" data-fragment-index=6>
    Normalmente, só é possível reduzir $\alpha(\theta)$ e $\beta(\theta)$ simultaneamente aumentando o tamanho amostral;
  </li>
  <li class="fragment" data-fragment-index=7>
    Depois de fixado o nível de significância e obtida a RC, se $H_1$ é composta, $\beta$ não possui um valor único e deve ser tratada como função de $\theta$.
  </li>
</ol>
...
Em linhas gerais, as etapas de um Teste de Hipóteses são:

<ol>
  <li>
    Estabelecer $H_0$ e $H_1$;
  </li>
  <li class="fragment" data-fragment-index=2>
    Observar o comportamento de $H_1$ e determinar o "formato" apropriado da RC;
  </li>
  <li class="fragment" data-fragment-index=3>
    Fixar o nível de significância $\sup_\theta\alpha(\theta)$;
  </li>
  <li class="fragment" data-fragment-index=4>
    Determinar a RC correspondente ao nível de significância estabelecido;
  </li>
  <li class="fragment" data-fragment-index=5>
    Verificar se a estimativa fornecida por $\hat{\theta}$ pertence à RC (o que nos leva a rejeitar $H_0$ ao nível de significância fixado). Se $\hat{\theta}$ não pertencer à RC, concluímos que a amostra não forneceu evidências suficientes para a rejeição de $H_0$.
  </li>
  <li class="fragment" data-fragment-index=6>
    Por que não podemos dizer que "aceitamos" $H_0$? [Pois não sabemos a probabilidade de erro ao fazermos tal afirmação!]{.fragment fragment-index=7}
  </li>
</ol>
...
<ol>
  <li>
    Um dos parâmetros de maior interesse prático é a média $\mu$ de uma população;
  </li>
  <li class="fragment" data-fragment-index=2>
    Podemos considerar as seguintes configurações clássicas de hipóteses a serem testadas:
    <ul>
      <li>
        $H_0 : \mu \leq \mu_0 \quad \times \quad H_1 : \mu > \mu_0$ (Unilateral à direita)
      </li>
      <li>
        $H_0 : \mu \geq \mu_0 \quad \times \quad H_1 : \mu < \mu_0$ (Unilateral à esquerda)
      </li>
      <li>
  ...
  <li class="fragment" data-fragment-index=3>
    Nas hipóteses acima, $\mu$ representa o parâmetro populacional (desconhecido), enquanto $\mu_0$ representa um valor numérico de referência (conhecido)
  </li>
</ol>

:::

## 3.1 Teste para $\mu$ {.unnumbered .unlisted}
...
**Caso 1:** Suponha que desejamos testar as hipóteses:
$$H_0 : \mu \leq \mu_0 \quad \times \quad H_1 : \mu > \mu_0$$
<ul>
  <li class="fragment" data-fragment-index=1>
    Podemos utilizar a média amostral $\bar{X}$ para construir a RC;
  </li>
  <li class="fragment" data-fragment-index=2>
    Perceba que, se $\bar{X}$ fornecer um valor muito maior que $\mu_0$, teremos fortes evidências para rejeitar $H_0$ em favor de $H_1$.
  </li>
  <li class="fragment" data-fragment-index=4>
    O valor crítico $x_c$ é o ponto que determina o que é muito maior do que $\mu_0$.
  </li>
  <li class="fragment" data-fragment-index=6>
    Portanto, $\text{RC} = \{\bar{X} > x_c\}$;
  </li>
  <li class="fragment" data-fragment-index=7>
    Como determinar $x_c$?
  </li>
...
#| fig-height: 4.5
#| echo: false
plot_base <- function(show_xc = FALSE) {
  par(mar = c(1.5, 1, 1.5, 1))
  plot(NULL, xlim = c(0, 10), ylim = c(-0.5, 3.2), axes = FALSE, xlab = "", ylab = "")
  arrows(x0 = 0.5, y0 = 0.5, x1 = 9.5, y1 = 0.5, length = 0.1, lwd = 1.5, code = 2)
  segments(x0 = 0.5, y0 = 2.4, x1 = 9.5, y1 = 2.4, lwd = 1)
  arrows(x0 = 0.5, y0 = 2.4, x1 = 0.3, y1 = 2.4, length = 0.08, code = 2)
  arrows(x0 = 9.5, y0 = 2.4, x1 = 9.7, y1 = 2.4, length = 0.08, code = 2)
  segments(x0 = 4, y0 = 2.3, x1 = 4, y1 = 2.5, lwd = 2)
  segments(x0 = 4, y0 = 0.3, x1 = 4, y1 = 2.5, lty = 2, col = "gray40")
  text(x = 4, y = -0.1, labels = expression(mu[0]), cex = 1.3)
  text(x = 2.25, y = 2.8, labels = expression(H[0]), cex = 1.5)
  segments(x0 = 4, y0 = 2.4, x1 = 9.5, y1 = 2.4, col = "white", lwd = 3)
  segments(x0 = 4, y0 = 2.4, x1 = 9.5, y1 = 2.4, lty = 2, lwd = 1.5)
  text(x = 6.75, y = 2.8, labels = expression(H[1]), cex = 1.5)
  if (show_xc) {
    segments(x0 = 6.5, y0 = 0.3, x1 = 6.5, y1 = 2.5, lty = 2, col = "gray40", lwd = 1.2)
    text(x = 6.5, y = -0.1, labels = expression(x[c]), cex = 1.3)
  }
}
# Passo 1: Ponto amostral próximo a mu_0 (Indício de aceitação de H_0)
plot_base(show_xc = FALSE)
points(x = 5.2, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 5.2, y = 0.2, labels = expression(bar(x)), cex = 1.3)
```
...
#| fig-height: 4.5
#| echo: false
# Passo 2: Ponto amostral distante de mu_0 (Forte indício de rejeição de H_0)
plot_base(show_xc = FALSE)
points(x = 8.2, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 8.2, y = 0.2, labels = expression(bar(x)), cex = 1.3)
```
...
#| fig-height: 4.5
#| echo: false
# Passo 3: Estabelecendo a fronteira da região de decisão x_c
plot_base(show_xc = TRUE)
```
...
#| fig-height: 4.5
#| echo: false
# Passo 4: Os dois cenários possíveis em relação a x_c
plot_base(show_xc = TRUE)
points(x = 5.2, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 5.2, y = 0.2, labels = expression(bar(x)), cex = 1.3)
points(x = 8.2, y = 0.5, pch = 19, cex = 1.8, col = "black")
...
Fixemos o nível de significância em $\alpha$ [e seja $z_{\alpha}$ o valor tal que $P(Z > z_{\alpha}) = \alpha$.]{.fragment fragment-index=1} [Pelo TCL, vem que
$$
\begin{aligned}
\alpha &= \sup \alpha(\mu) = \alpha(\mu_0) = P(\text{Rejeitar } H_0 \mid \mu = \mu_0) \\
&= \textstyle P(\bar{X} > x_c \mid \mu = \mu_0) = P\left( \frac{\bar{X} - \mu_0}{\sigma/\sqrt{n}} > \frac{x_c - \mu_0}{\sigma/\sqrt{n}} \right) \\
&\approx \textstyle  P\left( Z > \frac{x_c - \mu_0}{\sigma/\sqrt{n}} \right).
\end{aligned}
$$]{.fragment fragment-index=2}

[Logo, $\frac{x_c - \mu_0}{\sigma/\sqrt{n}} = z_{\alpha}$, isto é, $x_c = \mu_0 + z_{\alpha} \frac{\sigma}{\sqrt{n}}$.]{.fragment fragment-index=3} [Assim, rejeitamos $H_0$ ao nível de significância $\alpha$ se
$$
\textstyle \bar{X} > \mu_0 + z_{\alpha} \frac{\sigma}{\sqrt{n}}.
...
# 1. Configurando os parâmetros da distribuição
alpha <- 0.05
z_alpha <- qnorm(1 - alpha) # Calcula o ponto crítico (1.645)
...
# Customização do eixo x incluindo o ponto z_alpha
axis(1, at = c(-3, -2, -1, 0, 1, 2, 3), 
     labels = c("-3", "-2", "-1", "0", "1", "2", "3"), cex.axis = 1.1)
axis(1, at = c(z_alpha), 
     labels = c(expression(z[alpha])), cex.axis = 1, col.axis = "#3b5998", col = "#3b5998", col.ticks = "#3b5998")

# 3. Colorindo a região de rejeição (cauda superior)
x_shading <- seq(z_alpha, 3.5, length = 100)
y_shading <- dnorm(x_shading)
polygon(c(z_alpha, x_shading, 3.5), c(0, y_shading, 0), col = rgb(0.23, 0.35, 0.60, 0.4), border = NA)

# Linha vertical indicativa do z crítico
abline(v = z_alpha, lty = 2, col = "#3b5998", lwd = 1.5)

# 4. Adicionando rótulo para a área alpha
text(x = z_alpha + 0.3, y = 0.03, labels = expression(alpha), col = "#3b5998", font = 2, cex = 1.2)
...
**Caso 2:** Suponha que desejamos testar as hipóteses:
$$H_0 : \mu \geq \mu_0 \quad \times \quad H_1 : \mu < \mu_0$$
<ul>
  <li class="fragment" data-fragment-index=1>
    Podemos utilizar a média amostral $\bar{X}$ para construir a RC;
  </li>
  <li class="fragment" data-fragment-index=2>
    Perceba que, se $\bar{X}$ fornecer um valor muito menor que $\mu_0$, teremos fortes evidências para rejeitar $H_0$ em favor de $H_1$.
  </li>
  <li class="fragment" data-fragment-index=4>
    O valor crítico $x_c$ é o ponto que determina o que é muito menor do que $\mu_0$.
  </li>
  <li class="fragment" data-fragment-index=6>
    Portanto, $\text{RC} = \{\bar{X} < x_c\}$;
  </li>
...
plot_base <- function(show_xc = FALSE) {
  ...
  # H_1 fica à esquerda (tracejado), H_0 fica à direita (sólido)
  segments(x0 = 0.5, y0 = 2.4, x1 = 4, y1 = 2.4, col = "white", lwd = 3)
  segments(x0 = 0.5, y0 = 2.4, x1 = 4, y1 = 2.4, lty = 2, lwd = 1.5)
  ...
  text(x = 4, y = -0.1, labels = expression(mu[0]), cex = 1.3)
  text(x = 2.25, y = 2.8, labels = expression(H[1]), cex = 1.5)
  text(x = 6.75, y = 2.8, labels = expression(H[0]), cex = 1.5)
  
  if (show_xc) {
    segments(x0 = 1.8, y0 = 0.3, x1 = 1.8, y1 = 2.5, lty = 2, col = "gray40", lwd = 1.2)
    text(x = 1.8, y = -0.1, labels = expression(x[c]), cex = 1.3)
  }
}
# Passo 1: Ponto amostral próximo a mu_0 (Indício de não-rejeição de H_0)
plot_base(show_xc = FALSE)
points(x = 2.8, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 2.8, y = 0.2, labels = expression(bar(x)), cex = 1.3)
```
...
# Passo 2: Ponto amostral distante de mu_0 (Forte indício de rejeição de H_0)
plot_base(show_xc = FALSE)
points(x = 1.0, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 1.0, y = 0.2, labels = expression(bar(x)), cex = 1.3)
```
...
Para uma significância $\alpha$ [e $z_{\alpha}$ tal que $P(Z > z_{\alpha}) = \alpha$,]{.fragment fragment-index=1} [temos $P(Z < -z_\alpha) = \alpha$.]{.fragment fragment-index=2} [Pelo TCL, vem que
$$\begin{aligned}
\alpha &= \sup \alpha(\mu) = \alpha(\mu_0) = P(\text{Rejeitar } H_0 \mid \mu = \mu_0) \\
&= \textstyle P(\bar{X} < x_c \mid \mu = \mu_0) = P\left( \frac{\bar{X} - \mu_0}{\sigma/\sqrt{n}} < \frac{x_c - \mu_0}{\sigma/\sqrt{n}} \right) \\
&\approx \textstyle  P\left( Z < \frac{x_c - \mu_0}{\sigma/\sqrt{n}} \right).
...
[Logo, $\frac{x_c - \mu_0}{\sigma/\sqrt{n}} = -z_{\alpha}$, isto é, $x_c = \mu_0 - z_{\alpha} \frac{\sigma}{\sqrt{n}}$.]{.fragment fragment-index=4} [Assim, rejeitamos $H_0$ ao nível de significância $\alpha$ se
$$
\textstyle \bar{X} < \mu_0 - z_{\alpha} \frac{\sigma}{\sqrt{n}}.
$$]{.fragment fragment-index=5}
...
# 1. Configurando os parâmetros da distribuição
alpha <- 0.05
z_critico <- qnorm(alpha) # Calcula o ponto crítico (-1.645)

# Gerando o domínio para a curva normal padrão
x <- seq(-3.5, 3.5, length = 500)
y <- dnorm(x)

# 2. Desenhando o gráfico base (rejeição à esquerda)
plot(
  x, y, 
  type = "l", 
  lwd = 2.5, 
...
# Customização do eixo x incluindo o ponto crítico negativo
axis(1, at = c(-3, -2, -1, 0, 1, 2, 3), 
     labels = c("-3", "-2", "-1", "0", "1", "2", "3"), cex.axis = 1.1)
axis(1, at = c(z_critico), 
     labels = c(expression(-z[alpha])), cex.axis = 1, col.axis = "#3b5998", col = "#3b5998", col.ticks = "#3b5998")

# 3. Colorindo a região de rejeição (cauda inferior/esquerda)
x_shading <- seq(-3.5, z_critico, length = 100)
y_shading <- dnorm(x_shading)
polygon(c(-3.5, x_shading, z_critico), c(0, y_shading, 0), col = rgb(0.23, 0.35, 0.60, 0.4), border = NA)
...
# 4. Adicionando rótulo para a área alpha
text(x = z_critico - 0.3, y = 0.03, labels = expression(alpha), col = "#3b5998", font = 2, cex = 1.2)
...
**Caso 3:** Suponha que desejamos testar as hipóteses:
$$H_0 : \mu = \mu_0 \quad \times \quad H_1 : \mu \neq \mu_0$$
<ul>
  <li class="fragment" data-fragment-index=1>
    Vamos utilizar a média amostral $\bar{X}$ para construir a RC;
  </li>
  <li class="fragment" data-fragment-index=2>
    Perceba que, se $\bar{X}$ fornecer um valor muito diferente (muito menor ou maior) que $\mu_0$, teremos fortes evidências para rejeitar $H_0$ em favor de $H_1$.
  </li>
  <li class="fragment" data-fragment-index=4>
    Os valores críticos $x_{c_1}$ e $x_{c_2}$ delimitam o que é muito menor ou maior do que $\mu_0$.
  </li>
  <li class="fragment" data-fragment-index=6>
    Portanto, $\text{RC} = \{(\bar{X} < x_{c_1}) \cup (\bar{X} > x_{c_2})\}$;
  </li>
  <li class="fragment" data-fragment-index=7>
    Como determinar $x_{c_1}$ e $x_{c_2}$?
  </li>
</ul>  
...
plot_base <- function(show_xc = FALSE) {
  par(mar = c(1.5, 1, 1.5, 1))
  plot(NULL, xlim = c(0, 8), ylim = c(-0.5, 3.2), axes = FALSE, xlab = "", ylab = "")
  arrows(x0 = 0.5, y0 = 0.5, x1 = 7.5, y1 = 0.5, length = 0.1, lwd = 1.5, code = 2)
  # segments(x0 = 0.5, y0 = 2.4, x1 = 8.5, y1 = 2.4, lwd = 1)
  
  # H_1 fica à esquerda (tracejado), H_0 fica à direita (sólido)
  segments(x0 = 0.5, y0 = 2.4, x1 = 4, y1 = 2.4, col = "white", lwd = 3)
  segments(x0 = 0.5, y0 = 2.4, x1 = 4, y1 = 2.4, lty = 2, lwd = 1.5)
  segments(x0 = 7.5, y0 = 2.4, x1 = 4, y1 = 2.4, lty = 2, lwd = 1.5)
  
  arrows(x0 = 0.5, y0 = 2.4, x1 = 0.3, y1 = 2.4, length = 0.08, code = 2)
  arrows(x0 = 7.5, y0 = 2.4, x1 = 7.7, y1 = 2.4, length = 0.08, code = 2)
  segments(x0 = 4, y0 = 2.3, x1 = 4, y1 = 2.5, lwd = 2)
  segments(x0 = 4, y0 = 0.3, x1 = 4, y1 = 2.5, lty = 2, col = "gray40")
  
  text(x = 4, y = -0.1, labels = expression(mu[0]), cex = 1.3)
  text(x = 2.25, y = 2.8, labels = expression(H[1]), cex = 1.5)
  text(x = 5.75, y = 2.8, labels = expression(H[1]), cex = 1.5)
  text(x = 4, y = 2.8, labels = expression(H[0]), cex = 1.3)
  points(x = 4, y = 2.4, pch = 19, cex = 1.8, col = "black")
  
  if (show_xc) {
    segments(x0 = 1.8, y0 = 0.3, x1 = 1.8, y1 = 2.5, lty = 2, col = "gray40", lwd = 1.2)
    text(x = 1.8, y = -0.1, labels = expression(x[c[1]]), cex = 1.3)
    segments(x0 = 6.2, y0 = 0.3, x1 = 6.2, y1 = 2.5, lty = 2, col = "gray40", lwd = 1.2)
    text(x = 6.2, y = -0.1, labels = expression(x[c[2]]), cex = 1.3)
  }
}
# Passo 1: Ponto amostral próximo a mu_0 (Indício de não-rejeição de H_0)
plot_base(show_xc = FALSE)
...
points(x = 1.0, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 1.0, y = 0.2, labels = expression(bar(x)), cex = 1.3)
points(x = 7.0, y = 0.5, pch = 19, cex = 1.8, col = "black")
text(x = 7.0, y = 0.2, labels = expression(bar(x)), cex = 1.3)
```
...
Com significância $\alpha$ [e sendo $z_{\alpha}$ tal que $P(Z > z_{\alpha}) = \alpha$,]{.fragment fragment-index=1} [temos $P(Z < -z_\alpha) = \alpha$.]{.fragment fragment-index=2} [Do TCL, vem
$$\begin{aligned}
\alpha &= \sup \alpha(\mu) = \alpha(\mu_0) = P(\text{Rejeitar } H_0 \mid \mu = \mu_0) \\
&= \textstyle P\left[(\bar{X} < x_{c_1}) \cup (\bar{X} > x_{c_2}) \mid \mu = \mu_0 \right]\\
&= \textstyle P\left( \frac{\bar{X} - \mu_0}{\sigma/\sqrt{n}} < \frac{x_{c_1} - \mu_0}{\sigma/\sqrt{n}} \right) + P\left( \frac{\bar{X} - \mu_0}{\sigma/\sqrt{n}} > \frac{x_{c_2} - \mu_0}{\sigma/\sqrt{n}} \right) \\
&\approx \textstyle  P\left( Z < \frac{x_{c_1} - \mu_0}{\sigma/\sqrt{n}} \right) + P\left( Z > \frac{x_{c_2} - \mu_0}{\sigma/\sqrt{n}} \right).
\end{aligned}
$$]{.fragment fragment-index=3}
[Dividindo $\alpha$ igualmente, vem $\frac{x_{c_1} - \mu_0}{\sigma/\sqrt{n}} = -z_{\frac{\alpha}{2}}$ e $\frac{x_{c_2} - \mu_0}{\sigma/\sqrt{n}} = z_{\frac{\alpha}{2}}$.]{.fragment fragment-index=4} [Assim, rejeitamos $H_0$ ao nível $\alpha$ se
$$
\textstyle \bar{X} < x_{c_1} = \mu_0 - z_{\frac{\alpha}{2}} \frac{\sigma}{\sqrt{n}} \ \ \ \text{ou} \ \ \ \bar{X} > x_{c_2} = \mu_0 + z_{\frac{\alpha}{2}} \frac{\sigma}{\sqrt{n}}.
$$]{.fragment fragment-index=5}
...
# Customização do eixo x incluindo o ponto z_alpha
axis(1, at = c(-3, -2, -1, 0, 1, 2, 3), 
     labels = c("-3", "-2", "-1", "0", "1", "2", "3"), cex.axis = 1)
axis(1, at = c(z_alpha), 
     labels = c(expression(z[alpha])), cex.axis = 1, col.axis = "#3b5998", col = "#3b5998", col.ticks = "#3b5998")
...
axis(1, at = c(-3, -2, -1, 0, 1, 2, 3), 
     labels = c("-3", "-2", "-1", "0", "1", "2", "3"), cex.axis = 1)
axis(1, at = c(z_critico), 
     labels = c(expression(-z[alpha/2])), cex.axis = 1, col.axis = "#3b5998", col = "#3b5998", col.ticks = "#3b5998")
axis(1, at = c(-z_critico), 
     labels = c(expression(z[alpha/2])), cex.axis = 1, col.axis = "#3b5998", col = "#3b5998", col.ticks = "#3b5998")
...
x_shading <- seq(-z_critico, 3.5, length = 100)
y_shading <- dnorm(x_shading)
polygon(c(-z_critico, x_shading, 3.5), c(0, y_shading, 0), col = rgb(0.23, 0.35, 0.60, 0.4), border = NA)
...
# 4. Adicionando rótulo para a área alpha
text(x = z_critico - 0.3, y = 0.03, labels = expression(alpha/2), col = "#3b5998", font = 2, cex = .8)
text(x = -z_critico + 0.3, y = 0.03, labels = expression(alpha/2), col = "#3b5998", font = 2, cex = .8)
...
**Contextualização:** Os sistemas de escapamento de uma aeronave funcionam devido a um propelente sólido. A taxa de queima é uma característica importante desse produto.

[Suponha que:]{.fragment fragment-index=1}

<ul>
  <li class="fragment" data-fragment-index=1>
    as especificações exigem que a taxa de queima média seja de $50 \text{ cm/s}$;
  </li>
  <li class="fragment" data-fragment-index=2>
  de experiências anteriores, o desvio-padrão dessa variável pode ser tomado como $\sigma = 2 \text{ cm/s}$;
  </li>
  <li class="fragment" data-fragment-index=3>
  uma amostra de tamanho $n = 25$ forneceu taxa média de queima de $\bar{x} = 51\text{,}3 \text{ cm/s}$.
  </li>
</ul>

::: {.callout-important .fragment fragment-index=4 style="font-size: 1.428571em;"}
## Pergunta

Quais conclusões podem ser tiradas ao nível de significância de $\alpha = 0\text{,}05$?
:::
...
**Teste bilateral:** O problema nos leva a formular o teste das seguintes hipóteses:
$$
H_0 : \mu = 50 \quad \times \quad H_1 : \mu \neq 50.
$$
[**Dados:** Temos as seguintes informações: $\bar{x} = 51\text{,}3$; $\sigma = 2$; $n = 25$; $\alpha = 0\text{,}05$; e $\mu_0 = 50$.]{.fragment fragment-index=1}

[**Solução:** Como $\alpha = 0\text{,}05$ e o teste é bilateral, temos $z_{\frac{\alpha}{2}} = z_{0\text{,}025} = 1\text{,}96$.]{.fragment fragment-index=2} [Por sua vez, os valores críticos são
$$
\begin{aligned}
x_{c_1} &= \mu_0 - z_{\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}} = 50 - 1\text{,}96\left(\frac{2}{\sqrt{25}}\right) = 50 - 1\text{,}96(0\text{,}4) = 49\text{,}216 \\
x_{c_2} &= \mu_0 + z_{\frac{\alpha}{2}}\frac{\sigma}{\sqrt{n}} = 50 + 1\text{,}96\left(\frac{2}{\sqrt{25}}\right) = 50 + 1\text{,}96(0\text{,}4) = 50\text{,}784
\end{aligned}
$$]{.fragment fragment-index=3}

[**Decisão:** Como $\bar{x} = 51\text{,}3 > x_{c_2} = 50\text{,}784$, rejeitamos $H_0$ ao nível de significância de $5\%$.]{.fragment fragment-index=4}

## 3.1 Teste para $\mu$ {.unnumbered .unlisted}

<table style="width: 100%; border-collapse: collapse; margin-top: 20px; box-shadow: 0 4px 8px rgba(0,0,0,0.05); font-size: 0.75em; border-radius: 8px; overflow: hidden;">
  <thead>
    <tr style="background-color: #3b5998; color: white; text-align: left;">
      <th style="padding: 12px 15px; border: 1px solid #ddd;">Hipótese Alternativa ($H_1$)</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd;">Região Crítica / Rejeitar $H_0$ se:</th>
      <th style="padding: 12px 15px; border: 1px solid #ddd;">Valor(es) Crítico(s)</th>
    </tr>
  </thead>
  <tbody>
    <tr style="background-color: #fcfcfc;">
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$H_1 : \mu > \mu_0$ (unilateral à direita)</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd; color: #d9534f; font-weight: bold;">$\bar{X} > x_c$</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$x_c = \mu_0 + z_{\alpha} \frac{\sigma}{\sqrt{n}}$</td>
    </tr>
    <tr style="background-color: #f4f6f9;">
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$H_1 : \mu < \mu_0$ (unilateral à esquerda)</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd; color: #d9534f; font-weight: bold;">$\bar{X} < x_c$</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$x_c = \mu_0 - z_{\alpha} \frac{\sigma}{\sqrt{n}}$</td>
    </tr>
    <tr style="background-color: #fcfcfc;">
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$H_1 : \mu \neq \mu_0$ (bilateral)</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd; color: #d9534f; font-weight: bold;">
        $\bar{X} < x_{c_1}$ ou $\bar{X} > x_{c_2}$
      </td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">
        $x_{c_1} = \mu_0 - z_{\frac{\alpha}{2}} \frac{\sigma}{\sqrt{n}}$ <br>
        $x_{c_2} = \mu_0 + z_{\frac{\alpha}{2}} \frac{\sigma}{\sqrt{n}}$
      </td>
    </tr>
  </tbody>
</table>
...
<ol>
  <li>
    Os testes para $\mu$ se baseiam no TCL para aproximar a distribuição de $\frac{\bar{X} - \mu}{\sigma/\sqrt{n}}$ pela normal padrão. O que faz com que o nível de significância $\alpha$ seja aproximado no caso geral.
  </li>
  <li class="fragment">
    No caso geral, é necessário $n$ grande para uma aproximação satisfatória;
  </li>
  <li class="fragment">
    Se a população for normal, a aproximação do TCL não é necessária e o nível de significância é exato, independente do tamanho amostral;
  </li>
  <li class="fragment">
    Se $\sigma^2$ for desconhecida:
    <ul>
      <li class="fragment">
        e $n$ for suficientemente grande, podemos substituir $\sigma^2$ por $S^2$, pois a consistência de $S^2$ garante que a incerteza adicional provocada pela aproximação é negligenciável em grandes amostras;
      </li>
      <li class="fragment">
        e $n$ for pequeno, a incerteza adicional em substituir $\sigma^2$ por $S^2$ não pode ser desprezada.
      </li>
    </ul>
  </li>
</ol>
...
O que fazer se $\sigma^2$ for desconhecido e $n$ for pequeno?
:::
...
Como visto em Intervalos de Confiança, se $n$ for pequeno e a **população for normal**, $T = \frac{\bar{X} - \mu}{S/\sqrt{n}}$ tem distribuição $t_{n-1}$. Nesse caso, definindo novamente $t_{\alpha,\ \nu}$ como o valor tal que $P(T > t_{\alpha,\ \nu}) = \alpha$ e seguindo os mesmos passos para $\sigma^2$ conhecida, obtemos as seguintes RC e valores críticos correspondentes.
:::
...
<table style="width: 100%; border-collapse: collapse; margin-top: 20px; box-shadow: 0 4px 8px rgba(0,0,0,0.05); font-size: 0.75em; border-radius: 8px; overflow: hidden;">
  ...
  <tbody>
    <tr style="background-color: #fcfcfc;">
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$H_1 : \mu > \mu_0$ (unilateral à direita)</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd; color: #d9534f; font-weight: bold;">$\bar{X} > x_c$</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$x_c = \mu_0 + t_{\alpha,\ n-1} \frac{\sigma}{\sqrt{n}}$</td>
    </tr>
    <tr style="background-color: #f4f6f9;">
      <td style="padding: 12px 15px; border: 1px solid #ddd;">$H_1 : \mu < \mu_0$ (unilateral à esquerda)</td>
      <td style="padding: 12px 15px; border: 1px solid #ddd; color: #d9534f; font-weight: bold;">$\bar{X} < x_c$</td>
    ...
    <tr style="background-color: #fcfcfc;">
      ...
      <td style="padding: 12px 15px; border: 1px solid #ddd;">
        $x_{c_1} = \mu_0 - t_{\frac{\alpha}{2},\ n-1} \frac{\sigma}{\sqrt{n}}$ <br>
        $x_{c_2} = \mu_0 + t_{\frac{\alpha}{2},\ n-1} \frac{\sigma}{\sqrt{n}}$
      </td>
    </tr>
  </tbody>
</table>
...
![](figs/fig_tabela_tstudent_quantis_F01.drawio.png){width="100%" fig-align="center"}
...
Se $n$ for pequeno e a população não for normal, o TCL pode fornecer uma aproximação imprecisa. Nesse caso, podemos optar por testes Bootstrap (não abordados aqui) ou testes exatos.
:::

::: {.fragment style="border-style: solid; border-width: 3px; padding-bottom: 0px; padding-right: 12px; padding-left: 12px; font-size:70%;"}

**Exemplo 10 (cont.):** Vimos que $\frac{n\bar{X}}{\mu} \sim \text{Gama}(n, \frac{1}{\lambda})$ para população $\text{Exp}(\lambda)$. [Suponha que desejamos testar
$$H_0 : \mu \leq \mu_0 \quad \times \quad H_1 : \mu > \mu_0.$$]{.fragment}

[Teremos evidências para rejeitar $H_0$ se $\bar{X}$ fornecer um valor significativamente maior do que 4, o que induz à $\text{RC} = \{\bar{X} > x_c\}$.]{.fragment} [Agora, sendo $g_{p,n}$ tal que $P(\text{Gama}(n,1) \leq g_{p,n}) = p$, temos que
$$
\textstyle \alpha = \sup_\mu\alpha(\mu) = \alpha(\mu_0) = P(\bar{X} > x_c | \mu = \mu_0) = P(\frac{n\bar{X}}{\mu_0} > \frac{nx_c}{\mu_0}) = P(\text{Gama}(n,1) > \frac{nx_c}{\mu_0}).
$$]{.fragment}

[Isso implica em $P(\text{Gama}(n,1) \leq \frac{nx_c}{\mu_0}) = 1-\alpha$, logo $\frac{nx_c}{\mu_0} = g_{1-\alpha,n}$.]{.fragment} [Assim, rejeitamos $H_0$ se $\bar{X} > x_c = \frac{\mu_0}{n}g_{1-\alpha,n}$.]{.fragment}
...
No mesmo exemplo, vimos que uma aa de $n = 9$ bombas produziu $\hat\lambda = 0\text{,}237$, o que equivale a $\bar{X} = 1/\hat\lambda = 1/0\text{,}237 = 4\text{,}219$. [Suponha que desejamos testar
$$H_0 : \mu \leq 4 \quad \times \quad H_1 : \mu > 4.$$]{.fragment}

[Como discutido anteriormente, temos $\text{RC} = \{\bar{X} > x_c\}$, onde $x_c = \frac{\mu_0}{n}g_{1-\alpha,n}$.]{.fragment}

[Com confiança $\alpha = 0.05$, temos $g_{1-\alpha,n} = g_{0\text{,}95,\ 9} \approx 14$ (valor exato `r format(round(qgamma(p = .95, shape = 9, rate = 1), 3), decimal.mark = ",")`).]{.fragment} [Logo,
$$
x_c = \frac{\mu_0}{n}g_{1-\alpha,n} = \frac{4}{9}\cdot 14 = 6\text{,}222 \quad \text{(valor exato 6,416)}.
$$]{.fragment}

[Como $\bar{X} = 4\text{,}219 \not> x_c = 6\text{,}222$, concluímos que os dados não trazem evidências suficientes para rejeitar $H_0 : \mu \leq 4$ ao nível de significância de $\alpha = 0\text{,}05$.]{.fragment}

::: {.callout-caution .fragment style="font-size: 1.428571em;"}
## Observação

O desencadeamento lógico pode ser adaptado para testar hipóteses sobre o parâmetro $\lambda$ em vez de $\mu$.
:::
...
**Exemplo 14:** O tempo médio de alivio das dores do medicamento mais vendido no mercado é de 10 min. Uma empresa desenvolveu um medicamento e colocou a venda afirmando que um teste estatístico rejeitou $H_0 : \mu \geq 10$ em favor de $H_1 : \mu < 10$ ao nível de 0.1.
